...
const ContractAddress = "0x27db52d9431428917FfF3333DF19ab67e989cdf7";
const ABI = [
	{
		"inputs": [],
...
async function getHistoricalTokenPriceEvents() {
	...
	const filter = [ethers.utils.id('TokensBought(address,uint256,uint256,uint256)')];
...
async function fetchPrice() { //One token is worth this much (0.5)
    let price = await Contract.calculateTokenPrice();
    return ethers.utils.formatEther( price );
    //console.log(price);
}

// Buy
$('#b-tco').on('input', async function(e){
    const token = $("#b-tco").val();
    if (isNaN(token)) {
      alert("Please enter a valid number for Matic.");
      return;
    }
    $('#s-matic').html(token*await fetchPrice());
});

$('#btn-buy').click(async function(e){
	await buyTokens();
});

$('#btn-sell').click(async function(e){
	await sellTokens();
});
...
async function buyTokens() {
    ...
    const token = $("#b-tco").val();
    alert("You are about to buy " + token + " TCO for " + (token*price).toString() + " Matic.");
    const valueInWei = ethers.utils.parseEther((token*price).toString());
...
// Sell
$('#s-tco').on('input', async function(e){
    const token = $("#s-tco").val();
    if (isNaN(token)) {
        alert("Please enter a valid number for Matic.");
        return;
    }
    $('#g-matic').html(token*await fetchPrice());
});
...
async function sellTokens() {
    ...
    const transaction = await Contract.sellTokens(parseInt(token));
    await transaction.wait();
}